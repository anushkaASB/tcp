...
/*
// sign on response
struct MS_SIGNON_RESPONSE // 278 bytes
{
    MESSAGE_HEADER MESSAGE_HEADER_;
    int64_t UserID;
    char Reserved_1[8];
    ...
    char NewPassword[8];
    char TraderName[26];
    ap_int<32> LastPasswordChangeDate;
    char BrokerID[5];
    char Reserved_3[1];
    short BranchID;
    ap_int<32> VersionNumber;
    ap_int<32> EndTime;
    char Reserved_4[1];
    char Colour[50];
    char Reserved_5[1];
    ...
    char BrokerStatus[1];
    char ShowIndex[1];
    ST_BROKER_ELIGIBILITY_PER_MKT BROKER_ELIGIBILITY_PER_MKT;
    short MemberType;
...
class OrderEntry
{
  public:
    void operationPull(ap_uint<32> &regRxOperation,
                       hls::stream<orderEntryOperationPack_tt> &operationStreamPack,
                       hls::stream<orderEntryOperationPack_tt> &operationHostStreamPack,
                       hls::stream<orderEntryOperation_tt> &operationStream);

    void operationEncode(hls::stream<orderEntryOperation_tt> &operationStream,
...
extern "C" void orderEntryTcpTop(orderEntryRegControl_t &regControl,
                                 ...
                                 ap_uint<64> &GRResponseCapture,// not in tco ip
                                 ap_uint<64> &GRResponse,// not in tcp ip
                                 ap_uint<64> &serverProcessState,//not in tcp ip
                                 ap_uint<64> &operationProcessState,//not in tcp ip
